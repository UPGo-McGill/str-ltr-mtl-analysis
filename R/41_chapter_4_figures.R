...
fig_labels <-
  ...
  mutate(label = case_when(
    variable == "total listings" ~ 
      paste0(scales::comma(label), " ", variable, "\nin Jan/Feb"),
    variable == "active" ~ 
      paste0(scales::comma(label), " (", 
             scales::percent(label / sum(label) * 2, 0.1), ") ", variable, 
             "\nin December"),
    TRUE ~ 
      paste0(scales::comma(label), " (", 
             scales::percent(label / sum(label) * 2, 0.1), ") ", variable))) %>% 
...
figure_4_5_left <- 
  ...
  geom_line(lwd = 1) +
  scale_x_date(name = NULL) +
  scale_y_continuous(name = "Total daily reservations", 
                     limits = c(0, NA), 
                     label = scales::comma) +
