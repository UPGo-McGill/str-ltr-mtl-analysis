...
borough_housing_table %>% 
  ...
  rename(Arrondissement = borough,
         `Perte de logement (2019)` = housing_loss_2019,
         `Perte de logement (2018)` = housing_loss_2018,
...
annual_avg_rent %>% 
  ...
  mutate(across(c(active_strs, avg_rent_2015, avg_rent_2019, extra_rent), round, 
                -1)) %>% 
  set_names(c("Zone SCHL", "Annonces LCT actives quotidiennement (2019)", 
              "Loyer moyen (2015)", "Loyer moyen (2019)",
              "Augmentation totale des loyers (2015-2019)", 
              "Augmentation du loyer induite par les LCT (2015-2019)",
              "Part des LCT dans l'augmentation totale des loyers",
              "Loyer supplémentaire moyen payé en raison des LCT (2015-2019)")) %>% 
  gt() %>%
  tab_header(title = "L'impact des LCT sur les loyers") %>%
  opt_row_striping() %>% 
