...
bedroom_match_table <-
  ...
  slice(1:4) %>% 
  mutate(pct = round(units / sum(units), 3)) %>% 
  mutate(bedrooms = c("Studio", "1 chambre", "2 chambres", "3+ chambres")) %>% 
  select(bedrooms, pct) %>% 
  mutate(Category = "Parc locatif de la Ville de Montréal", .before = bedrooms)
...
bedroom_match_table <-
  ...
  mutate(bedrooms = c("Studio", "1 chambre", "2 chambres", "3+ chambres")) %>% 
  mutate(Category = "Marché des LCT (2019)", .before = bedrooms) %>% 
...
bedroom_match_table <-
  ...
  mutate(pct = round(pct / sum(pct), 3)) %>% 
  mutate(bedrooms = c("Studio", "1 chambre", "2 chambres", "3+ chambres")) %>% 
  mutate(Category = "Correspondances LCT-LLT", .before = bedrooms) %>% 
  bind_rows(bedroom_match_table, .)
...
bedroom_match_table <-
  ...
  mutate(pct = round(pct / sum(pct), 3)) %>% 
  mutate(bedrooms = c("Studio", "1 chambre", "2 chambres", "3+ chambres")) %>% 
  mutate(Category = "Non-correspondances", .before = bedrooms) %>% 
  bind_rows(bedroom_match_table, .)
...
bedroom_match_table %>% 
  gt() %>% 
  tab_header(title = "Annonces LCT et LLT par nombre de chambres") %>%
  opt_row_striping() %>% 
